...
public class SalesFunctions
{
        ...
        int id)
    {
        ...
        {
            var companyId = CompanyHelper.GetCompanyIdRequired(req);
            _logger.LogInformation("Emailing receipt for sale ID: {Id} for company {CompanyId}", id, companyId);

            var requestBody = await new StreamReader(req.Body).ReadToEndAsync();

            SendReceiptEmailRequest? request = null;
            if (!string.IsNullOrWhiteSpace(requestBody))
            {
                try
                {
                    request = JsonSerializer.Deserialize<SendReceiptEmailRequest>(requestBody, new JsonSerializerOptions
                    {
                        PropertyNameCaseInsensitive = true
                    });
                }
            ...
            {
                var errorResponse = ApiResponse<bool>.ErrorResponse("Email address is required");
                return await CreateHttpResponse(req, errorResponse, HttpStatusCode.BadRequest);
            }

            var response = await _salesService.EmailReceiptAsync(id, companyId, request.Email);
            ...
            else if (!string.IsNullOrEmpty(response.Message) && response.Message.Contains("not found", StringComparison.OrdinalIgnoreCase))
            {
