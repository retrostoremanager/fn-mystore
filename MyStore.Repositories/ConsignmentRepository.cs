    ...
    public async Task<ConsignmentItem> CreateAsync(ConsignmentItem item)
    {
        ...
        return await connection.QuerySingleAsync<ConsignmentItem>(
            $@"INSERT INTO consignment_item (
                company_id, customer_id, description, asking_price, split_percent, status, created_at)
               VALUES (
                @p_company_id, @p_customer_id, @p_description, @p_asking_price, @p_split_percent, @p_status, NOW())
