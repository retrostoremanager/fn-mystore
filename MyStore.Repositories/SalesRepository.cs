...
public class SalesRepository : ISalesRepository
{
    ...
    public async Task<List<Sale>> GetAllAsync(int companyId)
    {
        ...
        const string sql = @"SELECT id, company_id, customer_id, user_id, subtotal, 0 AS tax, subtotal AS total,
    ...
    public async Task<Sale> CreateAsync(Sale sale)
    {
        ...
        try
        {
            const string insertSale = @"INSERT INTO sale (company_id, customer_id, user_id, subtotal,
                    payment_method, sale_date, notes)
                VALUES (@CompanyId, @CustomerId, @UserId, @Subtotal, @PaymentMethod, @SaleDate, @Notes)
                RETURNING id";
